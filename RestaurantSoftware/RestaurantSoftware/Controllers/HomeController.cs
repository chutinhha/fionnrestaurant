...
namespace RestaurantSoftware.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            HttpCookie myCookie = Request.Cookies["UserSettings"];
            if (myCookie == null || myCookie["Role"] == "Customer")
            {
                RestaurantDatabaseEntities db = new RestaurantDatabaseEntities();
            ...
            else
            {
                if (aCookie["Role"] == "Staff")
                {
                    ...
                    if (order.generatedReceipt == 1)
                    {
                        ...
                        int customerID = Int32.Parse(order.customerID);
        ...
        [HttpPost]
        public ActionResult StaffLogin(string staffEmail, string staffPassword)
        {
            RestaurantDatabaseEntities db = new RestaurantDatabaseEntities();
            using (MD5 hash = MD5.Create())
            {
            ...
            SystemAdmin mySystemAdmin = db.SystemAdmins.FirstOrDefault(x => x.email == staffEmail && x.password == staffPassword);
            ...
            {
                HttpCookie myCookie = new HttpCookie("UserSettings");
                myCookie.Values["Email"] = mySystemAdmin.email;
